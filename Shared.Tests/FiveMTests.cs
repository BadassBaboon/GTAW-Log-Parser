...
namespace Shared.Tests
{
    public class FiveMTests
    {
        ...
        [Fact]
        public void FiveMConfigManager_GetExtendedTextureBudget_DefaultReturnsZero()
        {
            string nonExistentPath = Path.Combine(Path.GetTempPath(), "nonexistent_" + Guid.NewGuid().ToString("N"), "fivem.cfg");
            int budget = FiveMConfigManager.GetExtendedTextureBudget(nonExistentPath);
            Assert.Equal(0, budget);
        }

        [Fact]
        public void FiveMConfigManager_SetExtendedTextureBudget_UpdatesAndBypassesCapSuccessfully()
        {
            string tempDir = Path.Combine(Path.GetTempPath(), "FiveMTest_" + Guid.NewGuid().ToString("N"));
            try
            {
                ...
                string initialContent = "// generated by CitizenFX\n" +
                                        "unbindall\n" +
                                        "seta \"vid_budgetScale\" \"20\"\n" +
                                        "seta \"cam_vehicleFirstPersonFOV\" \"60.000000\"\n";
                File.WriteAllText(cfgPath, initialContent);

                // Read initial (20)
                int current = FiveMConfigManager.GetExtendedTextureBudget(cfgPath);
                Assert.Equal(20, current);

                // Update to 40 (Bypass cap)
                bool success = FiveMConfigManager.SetExtendedTextureBudget(40, out string statusMsg, cfgPath);
                Assert.True(success);
                Assert.Contains("40 (Bypass)", statusMsg);

                int updated = FiveMConfigManager.GetExtendedTextureBudget(cfgPath);
                Assert.Equal(40, updated);

                // Reset to 0 (Default)
                success = FiveMConfigManager.SetExtendedTextureBudget(0, out statusMsg, cfgPath);
                Assert.True(success);
                Assert.Contains("Default (0)", statusMsg);

                int resetVal = FiveMConfigManager.GetExtendedTextureBudget(cfgPath);
                Assert.Equal(0, resetVal);
            }
            finally
            {
                if (Directory.Exists(tempDir))
        ...
        [Fact]
        public void FiveMConfigManager_SetExtendedTextureBudget_CreatesNewCfgIfMissing()
        {
            string tempDir = Path.Combine(Path.GetTempPath(), "FiveMTest_" + Guid.NewGuid().ToString("N"));
            try
            {
                ...
                bool success = FiveMConfigManager.SetExtendedTextureBudget(60, out string statusMsg, cfgPath);
                Assert.True(success);
                Assert.True(File.Exists(cfgPath));

                int budget = FiveMConfigManager.GetExtendedTextureBudget(cfgPath);
                Assert.Equal(60, budget);
            }
            finally
            {
                if (Directory.Exists(tempDir))
        ...
        [Fact]
        public void AppSettingsManager_ParseLegacyUserConfigXml_ExtractsSettingsCorrectly()
        {
...
<configuration>
    <userSettings>
        ...
        [Fact]
        public void AppSettingsManager_ParseLegacyUserConfigXml_HandlesEmptyOrInvalidXmlGracefully()
        {
        ...
        public void AppSettingsManager_ConvertStringToType_ConvertsBasicTypesCorrectly(string input, Type targetType, object expected)
        {
        ...
        [Fact]
        public void AppSettingsManager_ConvertJsonElementToType_ConvertsTypesCorrectly()
        {
        ...
        [Fact]
        public void AppSettingsManager_SaveAndLoad_RoundTripsJsonFileProperly()
        {
